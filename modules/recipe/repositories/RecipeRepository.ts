...
export const RecipeRepository = (user: User): IRecipeRepository => {
  return {
    async exist(recipeId: string): Promise<boolean> {
    ...
    async isOwnedByUser(recipeId: string): Promise<boolean> {
      const recipe = await prisma.recipe.findFirst({
        where: {
          id: recipeId,
        },
      ...
      if (recipe.apiKeyId === user.apiKeyId) {
        return true;
      }
    ...
    async findById(recipeId: string): Promise<RecipeWithIngredients> {
      const recipe = await prisma.recipe.findFirst({
        where: {
          id: recipeId,
        },
